...
const btnMobileFilters = document.querySelector('button.btn-filters'),
	btnMobileFiltersClose = document.querySelector(
...
if (btnMobileFilters !== null) {
	mobileCategoriesHeading.forEach((item) => {
...
if (btnMobileFilters !== null) {
	mobileCategoriesItem.forEach((item) => {
		item.onclick = function () {
			const mobileFilters = document.querySelector('.catalog-sidebar');
			mobileFilters.classList.remove('show');
			document.querySelector('body').classList.remove('lock');
...
function getProducts(page = 1) {
	const formData = {
		...
		tags: $(
			'.catalog-sidebar .categories__item.checked,' +
				'.catalog-sidebar .categories__heading.checked'
		)
			.map((_, el) => {
				return $(el).data('id');
			})
			.get(),
		badges: $('.catalog-sidebar input[name="badges[]"]:checked')
			.map((_, el) => {
				return $(el).val();
			})
			.get(),
...
$(`.products input#search, .catalog-sidebar input[name="badges[]"]`).on(
	'change',
	getProducts
);
...
if (typeof container !== 'undefined' && container !== null) {
	let options = {
		...
		template: {
			...
			moveButton:
				'<a href="#" class="page-link page-link-{{type}}">' + '</a>',
			disabledMoveButton:
