...
const openOfficialDistributorsButton = document.getElementById(
	'official-distributor'
);
if (openOfficialDistributorsButton !== null) {
	const officialDistributorsModal = document.querySelector(
		'.modal--distributors'
	);
		...
		$.ajax({
			...
			success: function (response) {
				if (response.success) {
				  ...
				  `;
					$(
						'.product__slider--1 .swiper-wrapper, .product__slider--2 .swiper-wrapper'
					).each(function () {
						$(this).html(
							response.data.gallery.map(SwiperSlide).join('')
						);
					});
					Object.entries(response.data.info[0]).map((item) => {
						$(
							`.list-item[data-id=${item[0]}] .list-item__content, .list__item[data-id=${item[0]}] .list__item--content`
						).html(item[1]);
					});
					$this.addClass('is-active');
				}
